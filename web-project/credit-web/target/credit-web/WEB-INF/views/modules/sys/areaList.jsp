	...
	<script type="text/template" id="treeTableTpl">
		<tr id="{{row.id}}" pId="{{pid}}">
			...
			<shiro:hasPermission name="sys:area:edit"><td>
				<a class="icon-edit"  title="修改"      	 href="${ctx}/sys/area/form?id={{row.id}}"></a>
				<a class="icon-remove-circle" title="删除" href="${ctx}/sys/area/delete?id={{row.id}}" onclick="return confirmx('要删除该区域及所有子区域项吗？', this.href)"></a>
				<a class="icon-plus" title="添加下级区域" href="${ctx}/sys/area/form?parent.id={{row.id}}"></a> 
