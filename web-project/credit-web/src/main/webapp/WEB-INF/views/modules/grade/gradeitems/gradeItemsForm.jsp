	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#inputForm").validate({
				submitHandler: function(form){
					var validateFlag =0;
					$("#addOptionNew input[type=text]").each(function(){
							if($(this).val() == ''){
								 alertx("选项值名称或者选项分值不能为空")
								 validateFlag = 1;
								 $(this).focus();
							     return false;
							 }
		...
		this.addOption = function(id){
			...
			if(id=='addNew'){
				...
				var optionScore = "<input name='gradeOpt.score' type='text'  rows='4' cols='50' value='' class='required' />";
			...
			$.ajax({
						...
						success : function(data) {
							...
							if (modelList && modelList.length != 0) {
								$("select[id="+select+"]").append("<option value=''>--请选择--</option>");
...
<body>
	...
	<form:form id="inputForm" modelAttribute="gradeItems" action="${ctx}/grade/gradeitems/gradeItems/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls"><input class="btn btn-success" type="button" value="添加" onclick="addOption('addNew')"/></div><p id="erro"></p>
		</div>
						...
						<tbody>
							...
							<tr>
								...
								<td width="10%" colspan="3">
									<input maxlength="200"  type="text" rows="4" name="gradeOpt.score" class="required" value="${opt.score}"></input>
								</td>
