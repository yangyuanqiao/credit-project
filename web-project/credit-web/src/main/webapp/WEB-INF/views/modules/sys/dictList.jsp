		...
		<c:forEach items="${page.list}" var="dict">
			<tr>
				...
				<shiro:hasPermission name="sys:dict:edit"><td>
    				<a class="icon-edit"  title="修改"      	 href="${ctx}/sys/dict/form?id=${dict.id}"></a>
					<a class="icon-remove-circle" title="删除" href="${ctx}/sys/dict/delete?id=${dict.id}&type=${dict.type}" onclick="return confirmx('确认要删除该字典吗？', this.href)"></a>
    				<a class="icon-plus" title="添加键值" href="<c:url value='${fns:getAdminPath()}/sys/dict/form?type=${dict.type}&sort=${dict.sort+10}'><c:param name='description' value='${dict.description}'/></c:url>"></a>
