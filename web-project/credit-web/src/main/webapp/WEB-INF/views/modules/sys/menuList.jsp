			...
			<tbody><c:forEach items="${list}" var="menu">
				<tr id="${menu.id}" pId="${menu.parent.id ne '1'?menu.parent.id:'0'}">
					...
					<shiro:hasPermission name="sys:menu:edit"><td nowrap>
						<a class="icon-edit"  title="修改"      	 href="${ctx}/sys/menu/form?id=${menu.id}"></a>
						<a class="icon-remove-circle" title="删除" href="${ctx}/sys/menu/delete?id=${menu.id}" onclick="return confirmx('要删除该菜单及所有子菜单项吗？', this.href)"></a>
						<a class="icon-plus" title="添加下级菜单" href="${ctx}/sys/menu/form?parent.id=${menu.id}"></a> 
