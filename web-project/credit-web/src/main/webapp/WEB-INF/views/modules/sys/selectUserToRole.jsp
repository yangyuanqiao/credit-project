...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			djorgTree = $.fn.zTree.init($("#djorgTree"), setting, djorgNodes);
		...
		var djorgNodes=[
	            <c:forEach items="${djorgList}" var="djorg">
	            {id:"${djorg.id}",
	             pId:"${not empty djorg.parent?djorg.parent.id:0}", 
	             name:"${djorg.name}"},
	            </c:forEach>];
		...
		function treeOnClick(event, treeId, treeNode, clickFlag){
			...
			if("djorgTree"==treeId){
				$.get("${ctx}/sys/role/users?djorgId=" + treeNode.id, function(userNodes){
...
<body>
	<div id="assignRole" class="row-fluid span12">
		<div class="span4" style="border-right: 1px solid #A8A8A8;">
			<p>所在党组织：</p>
			<div id="djorgTree" class="ztree"></div>
		</div>
		<div class="span3">
			<p>待选人员：</p>
